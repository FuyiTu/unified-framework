...
DML_rf<-function(data){
  ...
  for(k in 1:numk){
    eval(parse(text = paste("indFoldk = Folds$Fold",k,sep = "")))
    indFoldkc = setdiff(1:n,indFoldk)
    fit1 = randomForest(data.Y~.,data = datafit[intersect(ind1,indFoldkc),])
    fit0 = randomForest(data.Y~.,data = datafit[intersect(ind0,indFoldkc),])
    ...
    for(i in 1:strt_num){
      ...
      fitk1 = randomForest(data.Y~.,data = datafit[intersect(indk1,indFoldkc),])
      fitk0 = randomForest(data.Y~.,data = datafit[intersect(indk0,indFoldkc),])
      indk = c(indk1,indk0)
      indkpre = intersect(indk,indFoldk)
...
DML_gbm<-function(data){
  ...
  for(k in 1:numk){
    eval(parse(text = paste("indFoldk = Folds$Fold",k,sep = "")))
    indFoldkc = setdiff(1:n,indFoldk)
    fit1 = gbm(data.Y~.,data = datafit[intersect(ind1,indFoldkc),],n.trees = 50,interaction.depth = 5,distribution = "gaussian")
    fit0 = gbm(data.Y~.,data = datafit[intersect(ind0,indFoldkc),],n.trees = 50,interaction.depth = 5,distribution = "gaussian")
    h1 = predict(fit1,data$X[indFoldk,])
    h0 = predict(fit0,data$X[indFoldk,])
    pait = length(which(data$A == 1))/n
    R[indFoldk] = R[indFoldk] - ((1-pait)*h1+pait*h0)
    for(i in 1:strt_num){
      indk1 = which(data$A == 1 & data$S == strt[i])
      indk0 = which(data$A == 0 & data$S == strt[i])
      fitk1 = gbm(data.Y~.,data = datafit[intersect(indk1,indFoldkc),],n.trees = 50,interaction.depth = 5,distribution = "gaussian")
      fitk0 = gbm(data.Y~.,data = datafit[intersect(indk0,indFoldkc),],n.trees = 50,interaction.depth = 5,distribution = "gaussian")
      indk = c(indk1,indk0)
      indkpre = intersect(indk,indFoldk)
      hk1 = predict(fitk1,data$X[indkpre,])
      hk0 = predict(fitk0,data$X[indkpre,])
...
DML_nnet<-function(data){
  ...
  for(k in 1:numk){
    eval(parse(text = paste("indFoldk = Folds$Fold",k,sep = "")))
    indFoldkc = setdiff(1:n,indFoldk)
    fit1 = nnet(data.Y~.,data = datafit[intersect(ind1,indFoldkc),],size = 2,decay = 0.02, maxit = 200,maxNwt = 1500)
    fit0 = nnet(data.Y~.,data = datafit[intersect(ind0,indFoldkc),],size = 2,decay = 0.02, maxit = 200,maxNwt = 1500)
    ...
    for(i in 1:strt_num){
      indk1 = which(data$A == 1 & data$S == strt[i])
      indk0 = which(data$A == 0 & data$S == strt[i])
      fitk1 = nnet(data.Y~.,data = datafit[intersect(indk1,indFoldkc),],size = 2,rang = 0.1,decay = 0.02, maxit = 200,maxNwt = 1500)
      fitk0 = nnet(data.Y~.,data = datafit[intersect(indk0,indFoldkc),],size = 2,rang = 0.1,decay = 0.02, maxit = 200,maxNwt = 1500)
      indk = c(indk1,indk0)
      indkpre = intersect(indk,indFoldk)
