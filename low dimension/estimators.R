...
est_gbm<-function(data){
  ...
  ind1 = which(data$A == 1)
  datafit = data.frame(data$Y,data$X)
  fit1 = gbm(data.Y~.,data = datafit[ind1,],distribution = "gaussian",n.minobsinnode = 3)
  fit0 = gbm(data.Y~.,data = datafit[ind0,],distribution = "gaussian",n.minobsinnode = 3)
  h1 = predict(fit1,data$X)
  h0 = predict(fit0,data$X)
  pait = length(which(data$A == 1))/n
  R = data$Y - ((1-pait)*h1+pait*h0)
  ...
  for(i in 1:strt_num){
    indk1 = which(data$A == 1 & data$S == strt[i])
    indk0 = which(data$A == 0 & data$S == strt[i])
    fitk1 = gbm(data.Y~.,data = datafit[indk1,],distribution = "gaussian",n.minobsinnode = 3)
    fitk0 = gbm(data.Y~.,data = datafit[indk0,],distribution = "gaussian",n.minobsinnode = 3)
    indk = c(indk1,indk0)
    hk1 = predict(fitk1,data$X[indk,])
    hk0 = predict(fitk0,data$X[indk,])
    pai[i] = length(indk1)/length(indk)
...
est_rf_high2<-function(data){
  ...
  ind1 = which(data$A == 1)
  datafit = data.frame(data$Y,data$X)
  fit1 = train(data.Y~.,data = datafit[ind1,],method = "rf")
  fit0 = train(data.Y~.,data = datafit[ind0,],method = "rf")
  h1 = predict(fit1,data$X)
  h0 = predict(fit0,data$X)
  pait = length(which(data$A == 1))/n
  R = data$Y - ((1-pait)*h1+pait*h0)
  ...
  for(i in 1:strt_num){
    indk1 = which(data$A == 1 & data$S == strt[i])
    indk0 = which(data$A == 0 & data$S == strt[i])
    fitk1 = train(data.Y~.,data = datafit[indk1,],method = "rf")
    fitk0 = train(data.Y~.,data = datafit[indk0,],method = "rf")
    indk = c(indk1,indk0)
    hk1 = predict(fitk1,data$X[indk,])
    hk0 = predict(fitk0,data$X[indk,])
    pai[i] = length(indk1)/length(indk)
...
est_rf<-function(data){
  ...
  ind1 = which(data$A == 1)
  datafit = data.frame(data$Y,data$X)
  fit1 = randomForest(data.Y~.,data = datafit[ind1,],ntree = 1000)
  fit0 = randomForest(data.Y~.,data = datafit[ind0,],ntree = 1000)
  h1 = predict(fit1,data$X)
  h0 = predict(fit0,data$X)
  pait = length(which(data$A == 1))/n
  R = data$Y - ((1-pait)*h1+pait*h0)
  ...
  for(i in 1:strt_num){
    indk1 = which(data$A == 1 & data$S == strt[i])
    indk0 = which(data$A == 0 & data$S == strt[i])
    fitk1 = randomForest(data.Y~.,data = datafit[indk1,],ntree=1000)
    fitk0 = randomForest(data.Y~.,data = datafit[indk0,],ntree=1000)
    indk = c(indk1,indk0)
    hk1 = predict(fitk1,data$X[indk,])
    hk0 = predict(fitk0,data$X[indk,])
    pai[i] = length(indk1)/length(indk)
